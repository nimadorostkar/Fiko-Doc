...
This guide explains all the filtering and search capabilities available in the Academy APIs.
...
- `ordering`: Sort by fields (`created_at`, `-created_at`, `video_hours`, `-video_hours`, `updated_at`, `-updated_at`)
...
- `video_hours`: Shortest first
- `-video_hours`: Longest first
...
1. **Language Filtering**: When filtering by language, only videos that have content (title or video file) in that language are returned.
